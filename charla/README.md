...
| Evidentia | No lanza o no termina | Enlace al resultado del ensayo en el panel del presentador |
...
Los respaldos viven en `datos/respaldos.json` y se **embeben** en la construcción: el Worker y la página del presentador leen exactamente los mismos.
...
`datos/config.json` lleva la URL pública (la del QR), la URL de Evidentia, el modelo y el precio por millón de tokens con el que se estima el costo de cada llamada (el real está en la consola de Anthropic).
...
Abra `/presentador` y escriba el token; queda una cookie de sesión (HttpOnly, `__Host-`, tres días). Para no escribirlo en tarima, guarde en el navegador del portátil el enlace `/presentador#t=<token>`: el fragmento nunca sale del navegador y se borra al usarse.
...
5. Descargue la copia sin red desde el panel (o `/presentador/descargar`) y ábrala desde el disco: los botones deben mostrar los resultados del ensayo con su fecha.
...
- Antes de subir: `/presentador`, **C**, «Lanzar la pregunta de la miel». Cierre el panel.
- Diapositiva 3: primer botón (Claude), después segundo botón (PubMed). Si a los 8 s no ha llegado nada, aparece el botón «Mostrar la respuesta del ensayo»; a los 25 s el respaldo entra solo.
- Diapositiva 10: **C** → «Abrir el resultado en vivo» (o el del ensayo si no terminó).
